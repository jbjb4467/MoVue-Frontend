...
export default {
  ...
  created: function () {
    ...
    axios.get(`http://3.21.166.3/movies/${this.$store.state.selectedMovie}/review/`, config)
      .then((res) => {
        this.reviews = res.data
  ...
  methods: {
    ...
    deleteReview: function (review, idx) {
      ...
      axios.delete(`http://3.21.166.3/movies/${this.$store.state.selectedMovie}/review/${review.id}`, config)
        .then((res => {
          console.log(res)
        }))
        .catch(err => console.log(err))
